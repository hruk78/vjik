...
//columns, row - max possible cols and raws
//returns int in double system 4=100 - only arab, 1=001 - only Zeom, 2=010 both of its
Cluster countClusterArbAndZm(FILE* fp, int *columns)
{
	Cluster cluster;
	...
	if ((getline(&line, &len, fp)) != -1) 
	{
		...
		if (tokens)
		{
			...
			for (i = 0; *(tokens + i); i++)
			{
				int IsArab=-3;
				IsArab=IsArabOrZea(*(tokens + i));
				if (IsArab == 0) cluster.ara+=1;
				if (IsArab == 1) cluster.zea+=1;
				if (IsArab < 0) printf("Something Strange at Col=%d, row=%d",col,row);
				free(*(tokens + i));
			}
			free(tokens);
	...
	//if both of flags [0] and [2] there are both of types of genes
	*columns=len_str;
	cluster.size=cluster.ara+cluster.zea;
	return cluster;
}
...
int main(int argc, char** argv) 
{
  ...
  ////////////////////////////
	{				
			...
			FILE* mf2=fopen(fileList[ii],"r");
			DeleteExt(fileList[ii]);
			asprintf(&outFile,"%s.stat.txt",fileList[ii]); 	
...
//count entire elem 
			...
			*/
			Cluster* cluster=(Cluster*)calloc((amount_str),sizeof(Cluster));
			...
			for( int i = 0; i < amount_str; ++i ) 
			{	
				cluster[currLine]=countClusterArbAndZm(mf2, &len_str);
			...
			fprintf(rf,"size\tN clusters\tArab\tZm\tarab%%\tarab+zm%%\tzm%%\n");
			for( int i = 1; i <= clust; i++ ) 
			{	
				...
				int ArabInClusterOfCizeN=0;
				int ZmInClusterOfCizeN=0;
				for( int j = 0; j < amount_str; ++j ) 
				{
					if (cluster[j].size == i)
					{
						ArabInClusterOfCizeN+=cluster[j].ara;
					    ZmInClusterOfCizeN+=cluster[j].zea;
					}
				}
fprintf(rf,"%d\t%d\t\t%d\t%d\t%.2f\t%.2f\t\t%.2f\n",i,distrib[i],ArabInClusterOfCizeN,ZmInClusterOfCizeN,ar,common,zm);
			}
			fprintf(rf,"\n");			
	...
	for( int ii = 0; ii < filesNum; ii++ ) free(fileList[ii]);
	free(fileList);
    //closedir(d);
	//fclose(Res);
