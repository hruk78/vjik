...
int main(int argc, char** argv) 
{
	...
	char *PositivePredFile=NULL;
	char *NegativePredFile=NULL;

//mediana search

TStColAndFloat gPosPairsPred;
TStColAndFloat gNegPairsPred;
memset(&gPosPairsPred, 0, sizeof(gPosPairsPred));
memset(&gNegPairsPred, 0, sizeof(gNegPairsPred));

//let us read weight sorted ortho and neg predictions
...
float k=1.0/RocSize;
// array RocSize
ConfMtrx=(struct ROC*)malloc(sizeof(struct ROC) * (RocSize));
memset(&ConfMtrx[0],0,sizeof(struct ROC));
memset(&ConfMtrx[RocSize-1],0,sizeof(struct ROC));

//0 and RocSize-1 elements are zeros
FILE* mf=fopen("ROC.txt","w");
for (int i=1; i<RocSize ; i++)
	{	
		memset(&ConfMtrx[i],0,sizeof(struct ROC));
		threshold=k*(float)i;
		printf("threshold=%1.3f\n",threshold);
		for (int a=0;a <= gPosPairsPred.size ; a++)
		{
			//asprintf(&(gPosPairsPredAfterTheshold.StrCol1[a]),"%s",gPosPairsPred.StrCol1[a]); 
			//asprintf(&(gPosPairsPredAfterTheshold.StrCol2[a]),"%s",gPosPairsPred.StrCol2[a]); 
			if (gPosPairsPred.weight[a] <= threshold )
			{
				//gPosPairsPredAfterTheshold.weight[a]=0;
				ConfMtrx[i].Fn++;
				
			}
			else
			{				
				//gPosPairsPredAfterTheshold.weight[a]=1;
				ConfMtrx[i].Tp++;
			}
		}
		for (int a=0;a <= gNegPairsPred.size ; a++)
		{
			//asprintf(&(gNegPairsPredAfterTheshold.StrCol1[a]),"%s",gNegPairsPred.StrCol1[a]); 
			//asprintf(&(gNegPairsPredAfterTheshold.StrCol2[a]),"%s",gNegPairsPred.StrCol2[a]); 
			if (gNegPairsPred.weight[a] <= threshold )
			{
				//gNegPairsPredAfterTheshold.weight[a]=0;
				ConfMtrx[i].Tn++;
			}
			else
			{				
				//gNegPairsPredAfterTheshold.weight[a]=1;
				ConfMtrx[i].Fp++;
			}
		}
		ConfMtrx[i].TPR=(float)(ConfMtrx[i].Tp)/((float)(gPosPairsPred.size));
        ConfMtrx[i].FPR=(float)(ConfMtrx[i].Fp)/((float)(gNegPairsPred.size));
		ConfMtrx[i].threshold=threshold;	
		fprintf(mf,"TPR=%1.3f FPR=%1.3f Threshold=%1.2f\n",ConfMtrx[i].TPR, ConfMtrx[i].FPR,ConfMtrx[i].threshold);
fprintf(mf,"       | Real P | Real N |\n");
fprintf(mf,"Pred P | Tp=%d%%  | Fp=%d%%  |\n",ConfMtrx[i].Tp/(ConfMtrx[i].Tp+ConfMtrx[i].Fn), ConfMtrx[i].Fp/(ConfMtrx[i].Fp+ConfMtrx[i].Tn));
fprintf(mf,"Pred N | Fn=%d%%  | Tn=%d%%  |\n",ConfMtrx[i].Fn/(ConfMtrx[i].Tp+ConfMtrx[i].Fn), ConfMtrx[i].Tn/(ConfMtrx[i].Fp+ConfMtrx[i].Tn));
fprintf(mf,"       |Tp+Fn=%d|Fp+Tn=%d|\n\n",(ConfMtrx[i].Tp+ConfMtrx[i].Fn),(ConfMtrx[i].Fp+ConfMtrx[i].Tn));
//fprintf(mf,"Pred P | Tp=%d  | Fp=%d  |\n",ConfMtrx[i].Tp, ConfMtrx[i].Fp);
//fprintf(mf,"Pred N | Fn=%d  | Tn=%d  |\n",ConfMtrx[i].Fn, ConfMtrx[i].Tn);
	}
