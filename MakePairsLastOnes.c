	...
	{	
		...
		for(int ii = 0; ii < src1->size_h ; ii++)
			...
			{	
				if (rws >=rows)
				{
					asprintf(&(gNamsToPredict->StrCol1[rws-rows]),"%s",src1->StrCol[ii]);
					asprintf(&(gNamsToPredict->StrCol2[rws-rows]),"%s",src2->StrCol[jj]);
				}
				rws=rws+1;
			}
	} 
	//gNamsToPredict->size=rws-number;
	//gNamsToPredict->StrCol1 = (char**)realloc(gNamsToPredict->StrCol1,(size_t)sizeof(char*) * ((size_t)(rws-number)));
	//gNamsToPredict->StrCol2 = (char**)realloc(gNamsToPredict->StrCol2,(size_t)sizeof(char*) * ((size_t)(rws-number)));
...
int main(int argc, char** argv) 
{
   ...
   if (getcwd(cwd, sizeof(cwd)) != NULL) {
	...
	printf("Read %s... Rows(amount_str)=%d Columns(len_str_max)=%d\n",species2File,amount_str2,len_str_max2);
