...
/////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
int WriteTwoKeyToFile(const char* Name,int columns, int rows, TStCol* gNamsToPredict, StrColIntMtrx *src1,StrColIntMtrx *src2)
{
	//Name new file
	//rows must be 1 000 000
	//columns - 2
	
	int rws=0;  //number of raws were added
	//printf("1036\n");
	int result=1;
	int dropped_str=0;
	char* toSearchFor=NULL;
	...
	gNamsToPredict->StrCol1 = (char**)malloc((size_t)sizeof(char*) * ((size_t)rows));
	gNamsToPredict->StrCol2 = (char**)malloc((size_t)sizeof(char*) * ((size_t)rows));
	gNamsToPredict->size=rows;
	{	
		rws=0;
		//printf("src1 strok %d, stolbcov %d ",src1->size_h,src1->size_l);
		//printf("src2 strok %d, stolbcov %d ",src2->size_h,src2->size_l);

		for(int ii = 0; ii < src1->size_h ; ii++)
			for(int jj = 0; jj < src2->size_h ; jj++)
			{	
				//printf("Src1 str =%s ",src1->StrCol[ii]);
				asprintf(&(gNamsToPredict->StrCol1[rws]),"%s",src1->StrCol[ii]);
				//printf("Src2 str =%s ",src2->StrCol[jj]);
				asprintf(&(gNamsToPredict->StrCol2[rws]),"%s",src2->StrCol[jj]);
				//printf(" Memory for %d\n",rws);
				rws=rws+1;
				if (rws >= rows) goto fin;
			}
	} 
fin:
	result=FPrintTwoStringStructure(Name, gNamsToPredict);
...
int main(int argc, char** argv) 
{
	//initialisation
		...
		//mkdir("iter_$iter/results/predictions");
		int PredNum; // how much strings for prediction to write to dmatrix
	...
	//else
	{
		//debug for not to set argv in the command line during debug
		//const char* n_argv[] = {"prdiction.c","ATH_TMAP_TGR_star.txt", "AMB_TMAP_TGR_star.txt", "Ath.Amb.pairs.txt",
		//"/home/hruk/prediction/iter_0/results/training/expression","/home/hruk/TEST_PELR_SCRIPT/iter_0/results"}; 
		//argv = n_argv;
		//argc = 6;
		...
		//asprintf(&modelDir, "%s", argv[4]);
		//iter_$iter/results/training/expression
		//asprintf(&PredDir, "%s", argv[4]);
...
/* temporary disabled to generate any pairs
	
	countTableElemFromFile(predictPairsFile, &len_str_max7, &amount_str7);
	//allocate memory for two string arrays
	//gNamsByFold[curr_fold].size=amount_str7;
	gNamsToPredict.StrCol1 = (char**)malloc(sizeof(char*) * (amount_str7));
	gNamsToPredict.StrCol2 = (char**)malloc(sizeof(char*) * (amount_str7));
	...
	ReadKnownTwoKeyColFromFile(predictPairsFile,gNamsToPredict.StrCol1,gNamsToPredict.StrCol2,&len_str_max7, &amount_str7,sp1Espr,sp2Espr);
	...
	//gNamsToPredict has two columns of names to predict wiht only pairs presence in sp1 and sp2
...
//disbled temporary for simple pairs generted debug purposes	
	...
	//memset(&trainSet, 0, sizeof(trainSet));	
 	...
 	//disbled temporary for simple pairs generted debug purposes
	//printf("Filed PosTrainList %d strings",FillTrainSet(&gNamsToPredict, &trainSet, sp1Espr, sp2Espr, "1"));
...
/* //disbled temporary for simple pairs generted debug purposes	
	...
	*/
	...
	//disbled temporary for simple pairs generted debug purposes	
	//printf("TrainSetArray address of a = %p\n", ( pTrainSetArr=MakeArrayTrainSet(&trainSet, pTrainSetArr)));
	
	//FPrintLblIntMtrx("TrainSetArray",pTestSetArr);			
	
/* //disbled temporary for simple pairs generted debug purposes
	
	//cycles begin
	//PredNum must be <= pTrainSetArr->raw
	if (PredNum > pTrainSetArr->raws) PredNum=pTrainSetArr->raws;
	//If PredNum is zero predict all raws, else the number from arguments
	if (!PredNum) PredNum=pTrainSetArr->raws;
*/

//int WriteTwoKeyToFile(const char* Name, int columns, int rows,struct StrColIntMtrx *src1,struct StrColIntMtrx *src2)
	
	printf("entering to WriteTwoKeyToFile");
	
	WriteTwoKeyToFile(predictPairsFile,2,PredNum,&gNamsToPredict,sp1Espr,sp2Espr);
