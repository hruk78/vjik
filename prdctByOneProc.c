...
#include <time.h>
//Depends on where c_api.h is installed
...
FPrintCOut("result.log","ReadTwoColFromFileMtoN 1179 \n");		
	(*columns)=len_str_max;
	//(*rows)=amount_str-dropped_str;
	printf("Added Rows %d ",(amount_str-dropped_str));
	//printf("READ of FILE to memory was good");
	fclose(fp);
	return amount_str-dropped_str;
}
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
...
void* PredictionAllByPortion(char* species1File,char* species2File,char* predictPairsFile,char* ModelName,
int MinIter,int MaxIter,int ReadPortion,int Nthread)
{
	...
	while (start < TotalRowsInFile+1)
{
	...
	if (end > TotalRowsInFile && start <= TotalRowsInFile)
		ReadPortion=TotalRowsInFile+1-start;
...
FPrintCOut("result.log","inside main cycle 1725\n");	
...
FPrintCOut("result.log","inside main cycle 1734\n");			
...
FPrintCOut("result.log","inside main cycle 1741\n");	
	t = time(NULL);
	tm = *localtime(&t);
	...
	printf("now: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		...
		//time 
FPrintCOut("result.log","inside main cycle 1753\n");			
		t = time(NULL);
		tm = *localtime(&t);
...
FPrintCOut("result.log","inside main cycle 1763\n");			
		t = time(NULL);
		tm = *localtime(&t);
		printf("Finish converting to DMatrix...\n");
		printf("now: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		...
		for (int iter=MinIter;iter <= MaxIter; iter++)
	{
		...
		//home/hruk/prediction/iter_0/results/training/expression/folds_all/model/
...
FPrintCOut("result.log","inside main cycle incice minIter_maxitet 1805\n");					
			...
			t = time(NULL);
			tm = *localtime(&t);
			printf("Predition started...\n");
			printf("now: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

		safe_xgboost(XGBoosterPredict(h_booster, h_train, 0, 0,0,&out_len, &out_result));
		//linux old ver xgboost may require obsolete function interfact
...
int main(int argc, char** argv) 
{
	//initialisation
		...
		MinIter=atoi(argv[4]);
		MaxIter=atoi(argv[5]); 
		ReadPortion=atoi(argv[6]);  //какими порциями обрабатывать файл пар
		Nthread=atoi(argv[7]);
