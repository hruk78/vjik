...
char* modelName, char* predictName,int ReadPortion,int Nthread)
{
	...
	//FPrintCOut("result.lS/og","inside main cycle incice minIter_maxitet 1805\n");					
...
int main(int argc, char** argv) 
{
	//initialisation
		...
		//mkdir("iter_$iter/results/predictions");
		int ReadPortion=0; // how much strings for prediction to write to dmatrix
		
		asprintf(&species1File, "%s", argv[1]);
		asprintf(&species2File, "%s", argv[2]);
		//asprintf(&predictPairsFile, "%s", argv[3]);  // dir/file with pairs to predict 
		//asprintf(&ModelName, "%s", argv[4]);  // dir/file with model to predict   
		// put predictions to /iter_N/results/predictions
		// orthopairs /iter_N/results/predictions/orthopairs
		// negative   /iter_N/results/predictions/negative
		//asprintf(&PredictName,"%s.predictions",ModelName); 
		
		MinIter=atoi(argv[3]);
		MaxIter=atoi(argv[4]); 
		ReadPortion=atoi(argv[5]);  //какими порциями обрабатывать файл пар
		Nthread=atoi(argv[6]);
	...
	char *OrthoPairFile=NULL;
	char *FullOrthoPairFile=NULL;
	char *PathOrthoPairFile=NULL;
	//char *ModelFile=NULL;
	char *FullModelFile=NULL;
	char *FullAllModelFile=NULL;
	//char *PathModelFile=NULL;
	//get from outer cycle
	char *PathIterN=NULL;
	char *FullPredictName=NULL;
	char *FullPredictNameAll=NULL;
	char *FullPredictNameFoeAndFr=NULL;
	char *OtherIterExtDir=NULL;
	char *PathNeg2=NULL;
	char *NegPairSumFile=NULL;
	char *NegPairSumFileFull=NULL;
	char *NegPairFile2=NULL;
	char *FullNegPairFile2=NULL;
	char *FoeDir=NULL;
	char *FoeDirNegFolds=NULL;
	char *FoeSumFile=NULL;
    char *FoeSumFileFull=NULL;
	
	//data generate
for (int iterExt=MinIter;iterExt <= MaxIter; iterExt++)
	{
	
	//asprintf(&PathIterN,"%s","iter_0");
	asprintf(&PathIterN,"iter_%d",iterExt);
	//asprintf(&ModelFile,"%s","model_1_1_1_1000.test");
	//create 
	char *PathOthoPredict=NULL;
	char *PathOtherN=NULL;
	asprintf(&PathOthoPredict,"%s/results/predictions/neg",PathIterN);
	//create new dir iter_N/results/predictions/neg
	struct stat st2 = {0};
	if (stat(PathOthoPredict, &st2) == -1) {
    mkdir(PathOthoPredict, 0700); 	}

	
	asprintf(&PathOrthoPairFile, "%s/%s",PathIterN,"results/data_for_learning/negative_folds/");
	asprintf(&FullPredictNameAll,"%s/results/predictions/neg/fold_all_friend.predictions",PathIterN);
	clean_file(FullPredictNameAll);
	//future nama of all others pairs to predict with all model
	
	for (int iterIn=0;iterIn <= 9; iterIn++)
	{
		//orthopair name
		asprintf(&OrthoPairFile, "fold_%d.orthopairs",iterIn);
		asprintf(&FullOrthoPairFile, "%s%s",PathOrthoPairFile,OrthoPairFile);
		// iter_0/results/data_for_learning/negative_folds/fold_0.orthopairs
		
		//model name
		asprintf(&FullModelFile, "%s/results/training/expression/folds_%d/model/model_1_1_1_1000.test",PathIterN,iterIn);
		// iter_0/results/training/expression/folds_0/model/model_1_1_1_1000.test
		asprintf(&FullAllModelFile, "%s/results/training/expression/folds_all/model/model_1_1_1_1000.test",PathIterN);
		// iter_0/results/training/expression/folds_all/model/model_1_1_1_1000.test
		
		//prediction name
		//just now only write, but must be simultaniesly read and sorted
		asprintf(&FullPredictName,"%s/results/predictions/neg/fold_%d.predictions",PathIterN,iterIn);
		
		//write to file consistent the model
		//it is good for debug, but for negative it is commented for optimizing, may be used, must work well
		//this is prediction by means of model fold_%d.predictions for pairs from /folds_N for negative pairs from thisj
		//external iteration
		//write summary preditictions to one file 
		printf("Friend pairs FullOrthoPairFile - %s , FullModelFile - %s , FullPredictNameAll - %s\n",FullOrthoPairFile,FullModelFile,FullPredictNameAll);
		PredictionAllByPortion(species1File,species2File,FullOrthoPairFile,FullModelFile,FullPredictNameAll,ReadPortion,Nthread);
		
		free(OrthoPairFile);
		free(FullOrthoPairFile);
		free(FullModelFile);
		free(FullAllModelFile);
		free(FullPredictName);
	
	}

	
	// iter_0/results/data_for_learning/negative_folds
	//asprintf(&FullPredictNameAllSum,"%s/results/data_for_learning/negative_folds/fold_all_friend.predictions",PathIterN);
			asprintf(&OtherIterExtDir,"iter_%d",iterExt);//
			asprintf(&PathNeg2, "%s/%s",OtherIterExtDir,"results/data_for_learning/negative_folds/");
			asprintf(&NegPairSumFile, "fold_all.orthopairs");
			asprintf(&NegPairSumFileFull,"%s%s",PathNeg2,NegPairSumFile);	

	clean_file(NegPairSumFileFull);
	//cycle with takes every iter_N 
	//for N not equal current iterExt_M it takes all negative pairs from iterExt_M and puts them to one file 
		for (int OtherIterExt=MinIter;OtherIterExt <= MaxIter; OtherIterExt++)
		{
			if (OtherIterExt != iterExt)
			{			
					asprintf(&FoeDir,"iter_%d",OtherIterExt);
					asprintf(&FoeDirNegFolds, "%s/%s",FoeDir,"/negative_sample/");
					asprintf(&FoeSumFile, "Negative_Sample.list");
					asprintf(&FoeSumFileFull,"%s%s",FoeDirNegFolds,FoeSumFile);	
				//printf("FullOrthoPairFile - %s , FullModelFile - %s , FullPredictName - %s\n",FullOrthoPairFile,FullModelFile,FullPredictName);
		
				//for (int iterIn2=0;iterIn2 <= 9; iterIn2++)
				{
				//orthopair name
				//asprintf(&NegPairFile2, "fold_%d.orthopairs",iterIn2);
				//asprintf(&FullNegPairFile2, "%s%s",PathNeg2,NegPairFile2);
				//FullNegPairFile2 iter_0/results/data_for_learning/negative_folds/fold_0.orthopairs
				//printf("Copy %s to %s , FullModelFile - %s , FullPredictNameAll - %s\n",FullNegPairFile2,NegPairSumFileFull);
				//append_file(FullNegPairFile2, NegPairSumFileFull);
				append_file(FoeSumFileFull, NegPairSumFileFull);
		...
		free(PathNeg2);
		//model name
		//asprintf(&FullAllModelFile, "%s/results/training/expression/folds_all/model/model_1_1_1_1000.test",OtherIterExt);
		// iter_0/results/training/expression/folds_all/model/model_1_1_1_1000.test
		...
		//free(NegPairSumFileFull);
		
		
		free(NegPairSumFile);
		//summary foe and friend negative
		asprintf(&FullPredictNameFoeAndFr,"%s/results/predictions/neg/foeAndfriend.predictions",PathIterN);
		clean_file(FullPredictNameFoeAndFr);
		//append friend predictions`
		append_file(FullPredictNameAll, FullPredictNameFoeAndFr);
		
		asprintf(&FullPredictNameAll,"%s/results/predictions/neg/fold_all_foe.predictions",PathIterN);
		clean_file(FullPredictNameAll);
		asprintf(&FullModelFile, "%s/results/training/expression/folds_all/model/model_1_1_1_1000.test",PathIterN,iterExt);
		printf("Foe pairs NegPairSumFileFull - %s , FullModelFile - %s , FullPredictNameAll - %s\n",NegPairSumFileFull,FullModelFile,FullPredictNameAll);
		PredictionAllByPortion(species1File,species2File,NegPairSumFileFull,FullModelFile,FullPredictNameAll,ReadPortion,Nthread);
		//free(FullAllModelFile);
	//read orthopredictions FullPredictName to the structure and qsort after all
	
	//summary foe and friend negative
	//asprintf(&FullPredictNameFoeAndFr,"%s/results/predictions/neg/foeAndfriend.predictions",PathIterN);
	//append foe predictions`
	append_file(FullPredictNameAll, FullPredictNameFoeAndFr);
	
	//buils sorted foe and friend reuslt
	TStColAndFloat gOrthoPairs; //to read once from iter_1 for example
	memset(&gOrthoPairs, 0, sizeof(gOrthoPairs));
	ReadTwoKeyColAndFloat(FullPredictNameFoeAndFr, &gOrthoPairs);
			
	
	//free two column and one float structure
	//FreeTStColAndFloat(&gOrthoPairs);
	char* FullPredictNameAllSort1;
	char* FullPredictNameAllSort2;
	char* FullPredictNameAllSort3;
	
	asprintf(&FullPredictNameAllSort1,"%s_sort1",FullPredictNameFoeAndFr);
	asprintf(&FullPredictNameAllSort2,"%s_sort2",FullPredictNameFoeAndFr);
	asprintf(&FullPredictNameAllSort3,"%s_sort3",FullPredictNameFoeAndFr);
	
	TSTFloatSort(&gOrthoPairs,1);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort1, &gOrthoPairs);
	
	TSTFloatSort(&gOrthoPairs,2);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort2, &gOrthoPairs);
	
	TSTFloatSort(&gOrthoPairs,3);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort3, &gOrthoPairs);
	
	FreeTStColAndFloat(&gOrthoPairs);
	free(FullPredictNameAll);
	free(FullPredictNameFoeAndFr);
	free(FullPredictNameAllSort1);
	free(FullPredictNameAllSort2);
	free(FullPredictNameAllSort3);
	
	free(PathOrthoPairFile);
	free(PathIterN);
	free(PathOthoPredict);
	//free(FullPredictNameAll);
	free(NegPairSumFileFull);
	free(FullModelFile);
	//free(FullPredictNameAll);
...
TStColAndFloat gOrthoPairs[MaxIter-MinIter+1]; //to read once from iter_1 for example
memset(&gOrthoPairs, 0, sizeof(gOrthoPairs));
...
for (int iterExt=MinIter;iterExt <= MaxIter; iterExt++)
	...
	asprintf(&PathIterN,"iter_%d",iterExt);
	char *PathOthoPredict=NULL;
	asprintf(&FullPredictNameAll,"%s/results/predictions/neg/foeAndfriend.predictions",PathIterN);
	
	
	//read orthopredictions FullPredictName to the structure and qsort after all
	memset(&gOrthoPairs[iterExt], 0, sizeof(gOrthoPairs[0]));
	ReadTwoKeyColAndFloat(FullPredictNameAll, &gOrthoPairs[iterExt]);
	...
	asprintf(&FullPredictNameAllSort1,"%s_sort1",FullPredictNameAll);
	asprintf(&FullPredictNameAllSort2,"%s_sort2",FullPredictNameAll);
	asprintf(&FullPredictNameAllSort3,"%s_sort3",FullPredictNameAll);
	
	TSTFloatSort(&gOrthoPairs[iterExt],1);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort1, &gOrthoPairs[iterExt]);
	
	TSTFloatSort(&gOrthoPairs[iterExt],2);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort2, &gOrthoPairs[iterExt]);
	
	TSTFloatSort(&gOrthoPairs[iterExt],3);
	FPrintTwoStringAndFloatStruct(FullPredictNameAllSort3, &gOrthoPairs[iterExt]);
	
	FreeTStColAndFloat(&gOrthoPairs[iterExt]);
	free(FullPredictNameAll);
	free(FullPredictNameAllSort1);
	free(FullPredictNameAllSort2);
	free(FullPredictNameAllSort3);
	...
	//free(PathOthoPredict);
		...
		//char* MedianaName
		//orthoPairMediana_sort1.txt
	mediana("foeAndfriend.predictions_sort1", "results/predictions/neg", "negMediana.txt",MinIter,MaxIter); 
