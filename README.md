...
Проект состоит из двенадцати скриптов:
1) makemodel.c - скрипт производит тренировку модели.
2) MakeAllPairs.с - генерация генных пар для предсказаний из набора данных об экспрессии генов в различных образцах. 
3) predictAllByPortion.с - скрипт предсказывает веса для генных пар, полученных скриптом (2) на основе моделей, полученных скриптом (1)
4) mediana.с - скрипт находит медиану предсказаний, полученных в (3) 
5) PositivPairsMedianaForROC.c - скрипт делает предсказания и строит медиану для ортопар. 
6) NegativPairsForRoc.с - скрипт делает предсказания и строит медиану для негативной выборки.
7) CopyPredToMedWithChange.с - скрипт объединяет медианы предсказаний для ортопар и для негативной выборки и вставляет в общую медиану предсказаний.
8) RocDataAuc.c - скрипт получает таблицу данных для построения ROC кривой предсказаний.
9) ClastersGeneCount.c - скрипт сбора статистики по кластеризации программой mcl. 
10) SeachInPred.c - скрипт быстрого поиска всех пар генов с весами для конкретного гена.
11) SeachInPredServer.c - сервер базы данных весов генных пар для быстрого поиска (10)
12) SeachInPredClient.c - клиент базы данных весов генных пар для быстрыго поиска (10)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!MUST DO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
Для дальнейшей работы над проектом необходимо
...
--------------------------далее выписки из рабочьего журнала, оставлены на всякий случай.
1)
