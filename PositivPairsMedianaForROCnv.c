...
for (int iterExt=MinIter;iterExt <= MaxIter; iterExt++)
	...
	asprintf(&PathOthoPredict,"%s/results/predictions/ortho",PathIterN);
	//create new dir iter_N/results/predictions/ortho
	struct stat st2 = {0};
	if (stat(PathOthoPredict, &st2) == -1) {
    mkdir(PathOthoPredict, 0700); 	}

	
	asprintf(&PathOrthoPairFile, "%s/%s",PathIterN,"results/data_for_learning/folds/");
	asprintf(&FullPredictNameAll,"%s/results/predictions/ortho/fold_all.predictions",PathIterN);
	
	for (int iterIn=0;iterIn <= 9; iterIn++)
	{
		//memset(&gNegPairs[iter], 0, sizeof(gNegPairs));
		//orthopair name
		asprintf(&OrthoPairFile, "fold_%d.orthopairs",iterIn);
		asprintf(&FullOrthoPairFile, "%s%s",PathOrthoPairFile,OrthoPairFile);
		// iter_0/results/data_for_learning/folds/fold_0.orthopairs
		...
		asprintf(&FullPredictName,"%s/results/predictions/ortho/fold_%d.predictions",PathIterN,iterIn);
		...
		PredictionAllByPortion(species1File,species2File,FullOrthoPairFile,FullModelFile,FullPredictName,ReadPortion,Nthread);
	...
	memset(&gOrthoPairs, 0, sizeof(gOrthoPairs));
	ReadTwoKeyColAndFloat(FullPredictNameAll, &gOrthoPairs);
...
for (int iterExt=MinIter;iterExt <= MaxIter; iterExt++)
	...
	//free(PathOthoPredict);
		...
		//char* MedianaName
		//orthoPairMediana_sort1.txt
	mediana("fold_all.predictions_sort1", "results/predictions/ortho", "orthoMediana.txt",MinIter,MaxIter); 
