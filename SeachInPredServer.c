...
int ReadKeyColAndIntTableFromFile(const char* Name, char** sp1Stopbec, int ** sp1Data,int *columns, size_t *rows)
{
	FILE* fp = fopen(Name, "r");
...
//////////////////////////////////////////////////////////////////////////
int ReadTwoKeyColAndFloatColFromFile(const char* Name, char** StrCol1, char** StrCol2, float *FloatData, int *columns, size_t *rows)
{
	FILE* fp = fopen(Name, "r");
	if (fp == NULL) exit(EXIT_FAILURE);
	int result=1;
...
///////////////////////////////////////////////////////////////////////////
int WriteTwoKeyColAndFloatColToFile(const char* Name, char** StrCol1, char** StrCol2, float *FloatData, size_t *amount_str)
{
	FILE* mf=fopen(Name,"w");
	if (mf == NULL) return 0;
	//else printf ("open for write done\n");
...
///////////////////////////////////////////////////////////////////////////
int WriteTwoKeyColAnd2dFloatColToFile(const char* Name, char** StrCol1, char** StrCol2, float *FloatData, size_t amount_str)
{
	FILE* mf=fopen(Name,"w");
	if (mf == NULL) return 0;
	//else printf ("open for write done\n");
...
void* TSTFloatSort(TStColAndFloat *gOrthoPairs,int key)
{
	...
	{
		asprintf(&(gOrthoPairs->StrCol1[a]),"%s",(gOrthoPairSt[a]).StrCol1);
		free((gOrthoPairSt[a]).StrCol1);
		asprintf(&(gOrthoPairs->StrCol2[a]),"%s",(gOrthoPairSt[a]).StrCol2); 
		free((gOrthoPairSt[a]).StrCol2);
...
//position - a position to be inserted new row
//start - from which to start to find. Both of of out arrays is acceding sorted, so no need to come bach to beginning
int FindKeyInTwoStrFloat(size_t start, const char* key1,const char* key2, struct TStColAndFloat *src, size_t* position)
{
	//int changeSign=0;
	char** pItem = NULL;
	if (!src || !key1 || !key2 || !src->StrCol1 || !src->StrCol2) return -1;
	//bsearch could not work because it is not array of structures, we have structure of arrays, bsearch finds any equal element
	//we need the first one, so we need to reverse till the first occurence. The second problem that if it fails to find, 
	//we have to find simply with cycle the place where key1 would be and return this position.
	size_t row1num=0;
	pItem = (char **)bsearch (key1, &(src->StrCol1[start]), (src->size)-start, sizeof(char*), cmpfunc2);
	//reverse to find first occurence
    if (pItem != NULL) //find OK
	{		
		if (pItem-&(src->StrCol1[start]) > 0)
		{			
			row1num = (size_t)(pItem-&(src->StrCol1[start]))+start;	
			for (size_t i =row1num+1; i >0; --i ) 
			{
				if ( strcmp(src->StrCol1[i-1], key1) != 0 ) 
				{
					row1num=i;
					break;
				}
	...
	{
		//one have to find simply with cycle the place where key1 would be
		for (size_t i =start; i < src->size; ++i ) 
		{
			if ( strcmp(src->StrCol1[i], key1) > 0 )
			{
				//occurence is not found, row must be inserted
				*position = i;
				return -1;
			} 
			else 
			{ 	//must be inserted to the end
				if (i == src->size-1)
				{
					*position = i;
					return -1;
	...
	//bool found=FALSE;
	for (size_t i =row1num; i < src->size; ++i ) 
	{
		if (!strcmp(src->StrCol1[i], key1))
		{
			if (!strcmp(src->StrCol2[i], key2))
			{
				//key2 found ;
				*position = i;
				return 0;
			}
			else if (strcmp(src->StrCol2[i], key2) < 0)
			{
				//do nothing current str is lower then possible key2 position. Proceed scanning.
			}
			else if (strcmp(src->StrCol2[i], key2) > 0)
			{
				//there is no key2 in sorted str col2, i - is a key2 future position to insert key2
				*position = i;
				//key2 is not found
				return -1;
			}
		}
		else //if (strcmp(src->StrCol2[i], key2))
		{
			//key2 not found and key1 is changed. Not important. Key1 key2 weight must be inserted anyway
			*position = i;
			return -1;
		}
	}
...
///////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
//Find key in src->StrCol
//src->StrCol must be accending sorted
//returns 0 - found -1 not found
//position - a position to be inserted new row
//start - from which to start to find. Both of of out arrays is acceding sorted, so no need to come bach to beginning
int FindKeyIn1ofTwoStrFloat(size_t offset, const char* key1, struct TStColAndFloat *src, size_t* position)
{
	char** pItem = NULL;
	if (!src || !key1 || !src->StrCol1) return -1;
	size_t row1num=0;
	pItem = (char **)bsearch (key1, &(src->StrCol1[offset]), (src->size)-offset, sizeof(char*), cmpfunc2);
	//reverse to find first occurence
    if (pItem != NULL) //find OK
	{		
		if (pItem-&(src->StrCol1[offset]) > 0)
		{			
			int i =  (size_t)(pItem-&(src->StrCol1[offset]))+offset;	
			while ( i > offset && strcmp(src->StrCol1[i-1], key1) == 0 )
			{
				i=i-1;
			}
			row1num=i;
...
///////////////////////////////////////////////////////////////////////////
int FindKeyIn2ofTwoStrFloat(size_t offset, const char* key1, struct TStColAndFloat *src, size_t* position)
{
	char** pItem = NULL;
	if (!src || !key1 || !src->StrCol2) return -1;
	size_t row1num=0;
	pItem = (char **)bsearch (key1, &(src->StrCol2[offset]), (src->size)-offset, sizeof(char*), cmpfunc2);
	//reverse to find first occurence
    if (pItem != NULL) //find OK
	{		
		if (pItem-&(src->StrCol2[offset]) > 0)
		{			
			int i =  (size_t)(pItem-&(src->StrCol2[offset]))+offset;	
			while ( i > offset && strcmp(src->StrCol2[i-1], key1) == 0 )
			{
				i=i-1;
			}
			row1num=i;
...
//Find key in src->StrCol
//src->StrCol must be accending sorted
//returns NULL if no results, otherwise array int[], int[0] - size of int[], [1;int[0]] - results int array
//position - a position to be inserted new row
//Both of of out arrays is acceding sorted, so no need to come bach to beginning
int* FindKeySInUnsorted2TwoStrFloat(const char* key1, struct TStColAndFloat *src)
{
	int N=1000; //initial size of int[] of results
	int Nres=1; //resulted size of int[] of results, 1 from the very beginnin contains results
	int *res=(int*)malloc(sizeof(int*) * (N));
	res[0]=0; //no results
	
	char** pItem = NULL;
	if (!src || !key1 || !src->StrCol1 || !src->StrCol2) return NULL;
	
	//find the range of equal element in first col
	int st=0;
	int j=st; //absolute distance from [0]
	int p; //flag to finish cycle
	//start of the cycle
	while (j < src->size)
	{	
		if (Nres > N-1) 
		{
			N=N+1000;
			res=(int*)realloc(res, sizeof(int*) * (N));
		}
		while ( (p=(j+1 < src->size) ) && 
			   (src->StrCol1[j])[(strlen(src->StrCol1[j]))-1] == (src->StrCol1[j+1])[(strlen(src->StrCol1[j+1]))-1] &&
			   (src->StrCol1[j])[(strlen(src->StrCol1[j]))-2] == (src->StrCol1[j+1])[(strlen(src->StrCol1[j+1]))-2] &&
			   (src->StrCol1[j])[(strlen(src->StrCol1[j]))-3] == (src->StrCol1[j+1])[(strlen(src->StrCol1[j+1]))-3])	
		{
			j=j+1;
		}
		//range [st;j-1] to bsearch
		pItem = (char **)bsearch (key1, &(src->StrCol2[st]), j-st+1, sizeof(char*), cmpfunc2);
		if (!pItem) {st=j+1; j=st; continue;}
		//append found elem to array
		res[Nres]=(pItem-&(src->StrCol2[st]))+st;
		res[0]=Nres+1;
		Nres=Nres+1;
		if (!p) break; 
		st=j+1;
	...
	//reduce result array, if there is unused allocations or delete
	if (Nres > 1)
	{
		res=(int*)realloc(res, sizeof(int*) * (Nres));
	}
	else
	{
		free(res);
...
///////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////
//size_t size insert one row str1 str2 float
//it is supposed that the row number is already counted, but it is possible
//to implement this function for adding new elements to structure
struct TStColAndFloat* InsertRowInTwoStringFloat(char* StrCol1, char* StrCol2, float weight,struct TStColAndFloat *src, size_t pos)
{
	size_t add= src->size+1; 
	TStColAndFloat* temp=(TStColAndFloat*)malloc(sizeof(TStColAndFloat));
	memset(temp,0,sizeof(TStColAndFloat));
	temp->StrCol1 = (char**)malloc(sizeof(char*) * (add));
	temp->StrCol2 = (char**)malloc(sizeof(char*) * (add));
	temp->weight=(float*)malloc(sizeof(float) * (add));
	temp->size=add;
	
	if (!temp || !(temp->StrCol1) || !(temp->StrCol2) || !(temp->weight) )
	printf("InsertRowInTwoStringFloat is lack of memory");
	
	//dst->size= *amount_str;
	//insert elements before pos	
	for (size_t i = 0; i<pos; i++)
	{
		asprintf(&(temp->StrCol1[i]),"%s",src->StrCol1[i]); 
		asprintf(&(temp->StrCol2[i]),"%s",src->StrCol2[i]); 
		temp->weight[i]=src->weight[i];
	}
	//
	asprintf(&(temp->StrCol1[pos]),"%s",StrCol1); 
	asprintf(&(temp->StrCol2[pos]),"%s",StrCol2); 
	temp->weight[pos]=weight;
	
	for (size_t i = pos+1; i<add; i++)
	{
		asprintf(&(temp->StrCol1[i]),"%s",src->StrCol1[i-1]); 
		asprintf(&(temp->StrCol2[i]),"%s",src->StrCol2[i-1]); 
		temp->weight[i]=src->weight[i-1];
	}
	
	FreeTStColAndFloat(src);	
	//free(src);
	return temp;
}
///////////////////////////////////////////////////////////////////////////
int InsertRowInTwoStringFloat2(char* StrCol1, char* StrCol2, float weight,struct TStColAndFloat *src, size_t pos)
{
	size_t add= src->size+1; 
	//TStColAndFloat* temp=(TStColAndFloat*)malloc(sizeof(TStColAndFloat));
	//memset(temp,0,sizeof(TStColAndFloat));
	src->StrCol1 = (char**)realloc(src->StrCol1,sizeof(char*) * (add));
	src->StrCol2 = (char**)realloc(src->StrCol2,sizeof(char*) * (add));
	src->weight=(float*)realloc(src->weight,sizeof(float) * (add));
	src->size=add;
	
	if (!src || !(src->StrCol1) || !(src->StrCol2) || !(src->weight) )
	printf("InsertRowInTwoStringFloat is lack of memory");
	...
	//for (size_t i = 0; i<pos; i++)
	//{
	//	asprintf(&(src->StrCol1[i]),"%s",src->StrCol1[i]); 
	//	asprintf(&(src->StrCol2[i]),"%s",src->StrCol2[i]); 
	//	src->weight[i]=src->weight[i];
	//}
	//
	
	for (size_t i = add-1; i>pos; i--)
	{
		asprintf(&(src->StrCol1[i]),"%s",src->StrCol1[i-1]); 
		free(src->StrCol1[i-1]);
		asprintf(&(src->StrCol2[i]),"%s",src->StrCol2[i-1]); 
		free(src->StrCol2[i-1]);
		src->weight[i]=src->weight[i-1];
	}
	
	asprintf(&(src->StrCol1[pos]),"%s",StrCol1); 
	asprintf(&(src->StrCol2[pos]),"%s",StrCol2); 
	src->weight[pos]=weight;
...
///////////////////////////////////////////////////////////////////////////
// search in prediction all genes pairs consistent the one gene with thrashold larger
//first - prediction pairs with weigh file
//second - gene name to look paris with
//third - weight
//File name with resulat is gene name with weight
///////////////////////////////////////////////////////////////////////////
int search_gene_pairs(char* source,char* destin,char *thrashold) 
{
	size_t postion=0, a=0;
	//file with genes predictions
	//asprintf(&source, "%s", argv[1]);
	//gene name to find in future possible list of genes
	//asprintf(&destin, "%s", argv[2]);
	//float thrashold=(float)atof(argv[3]);
	float thrasholdf;
	DeleteLastN(destin);
	char * destinFile;
	thrasholdf=atof(thrashold);
	asprintf(&destinFile,"%s_%s",destin,thrashold); 
	TStColAndFloat sourceData;
	memset(&sourceData, 0, sizeof(sourceData));
	//let us read 
	ReadTwoKeyColAndFloat(source, &sourceData);
	FILE* mf=fopen(destinFile,"w");
	if (mf == NULL) return 0;
	size_t position;
	//int FindKeyIn2ofTwoStrFloat(size_t offset, const char* key1, struct TStColAndFloat *src, size_t* position)
	if (FindKeyIn1ofTwoStrFloat(0, destin, &sourceData, &position) >=0 )
	{
		//found
			a=position;
			while (a<sourceData.size && !strcmp(sourceData.StrCol1[a],destin))
			{
				if (sourceData.weight[a] >= thrasholdf)
				{
				//fprintf(mf,"%s\t",StrCol1[a]);
				fprintf(mf,"%s\t",sourceData.StrCol2[a]);
				fprintf(mf,"%.2f\n",sourceData.weight[a]);
				}
				a=a+1;
			}
	...
	{
		TSTFloatSort(&sourceData,2);
		if (FindKeyIn2ofTwoStrFloat(0, destin, &sourceData, &position) >=0 )
		{
			size_t a=position;
			while (a<sourceData.size && !strcmp(sourceData.StrCol2[a],destin))
			{
				if (sourceData.weight[a] >= thrasholdf)
				{
				fprintf(mf,"%s\t",sourceData.StrCol1[a]);
				//fprintf(mf,"%s\t",StrCol2[a]);
				fprintf(mf,"%.2f\n", sourceData.weight[a]);
				}
				a=a+1;
...
//File name with resulat is gene name with weight
///////////////////////////////////////////////////////////////////////////
int search_gene_pairs_in_memory(TStColAndFloat *sourceData,char* destin,char *thrasholdStr) 
{
	size_t postion=0, a=0;
	//file with genes predictions
	//asprintf(&source, "%s", argv[1]);
	//gene name to find in future possible list of genes
	//asprintf(&destin, "%s", argv[2]);
	float thrashold=atof(thrasholdStr);
	DeleteLastN(destin);
	char * destinFile;
	asprintf(&destinFile,"%s_%s",destin,thrasholdStr); 
	...
	if (FindKeyIn1ofTwoStrFloat(0, destin, sourceData, &position) >=0 )
	{
		//found
			...
			while (a<sourceData->size && !strcmp(sourceData->StrCol1[a],destin))
			{
				if (sourceData->weight[a] >= thrashold)
				{
				//fprintf(mf,"%s\t",StrCol1[a]);
				fprintf(mf,"%s\t",sourceData->StrCol2[a]);
				fprintf(mf,"%.2f\n",sourceData->weight[a]);
	...
	} else
	{
		...
		if (FindKeyIn2ofTwoStrFloat(0, destin, sourceData, &position) >=0 )
		{
			...
			//change StrCol1 and StrCol2 to let user seach gene of z***	type by means of the first part 
			//of this if.
			char **temp;
			temp=sourceData->StrCol1;
			sourceData->StrCol1=sourceData->StrCol2;
			sourceData->StrCol2=temp;
		}
	}		
	fclose(mf);
	free(destinFile);
...
///////////////////////////////////////////////////////////////////////////
//without second column resort, a bit slower in average, but quick to on the first iteration
//and for the case of silly user`s changing of gene types (AT*** and Z***)
///////////////////////////////////////////////////////////////////////////////
int search_gene_pairs_in_memory2(TStColAndFloat *sourceData,char* destin,char *thrasholdStr) 
{
	size_t postion=0, a=0;
	//file with genes predictions
	//asprintf(&source, "%s", argv[1]);
	...
	} else
	{
		...
		//position - a position to be inserted new row
		//Both of of out arrays is acceding sorted, so no need to come bach to beginning
		if ( (res=FindKeySInUnsorted2TwoStrFloat(destin, sourceData)) )
		{
			//a=1 a=0 !array size!
			while (a < res[0])
			{
				
			if (sourceData->weight[ res[a] ] >= thrashold)
				{
				fprintf(mf,"%s\t",sourceData->StrCol1[res[a]]);
				//fprintf(mf,"%s\t",StrCol2[a]);
				fprintf(mf,"%.2f\n", sourceData->weight[res[a]]);
				}
				a=a+1;
			}
			free(res);
...
{
	//first parametr newPred to put
	//second paramenr old pred to change with newPred
	int n=0;
	size_t postion=0;
	char* source=NULL;
	char* destin=NULL;
       ...
       while(1)
    {
		...
		//writes to connfd, which sends it to client
        n = write(connfd, sendBuff, strlen(sendBuff)); 
		if (n < 0) error("ERROR writing to socket");
		memset(sendBuff, '0', sizeof(sendBuff)); 
		n = read( connfd,sendBuff, (strlen(sendBuff)));
		
		if (n < 0) printf("server: wait\t");
		//else printf("server: gene weight %s\n",sendBuff);
		//sendBuff to file.txt\tGeneName\tweight
		{
			char** tokens;
			tokens = str_split(sendBuff, '\t', &len_str);
			//remeber max line length
			if (tokens)
			{
				int ii;
				for (ii = 0; *(tokens + ii); ii++)
				{
					if (ii==0) 
					{
						if (!source) 
						{
							asprintf(&source,"%s",*(tokens + ii));
							//read file with genes predictions
							//let us read 
							ReadTwoKeyColAndFloat(source, &sourceData);
						...
						{
							//must sravnit` stary i novy faily esli razlichauutsa, to zagrusit snova
						}
					}
					if (ii==1) asprintf(&destin,"%s",*(tokens + ii));
					if (ii==2) 
					{	
						thrashold=atof(*(tokens + ii));
						asprintf(&thrasholdStr,"%s",*(tokens + ii));
						printf("server: file %s gene %s weight %s processing...\n",source,destin,thrasholdStr);
						//search_gene_pairs(source,destin,thrasholdStr);
						search_gene_pairs_in_memory2(&sourceData,destin,thrasholdStr); 
						printf("Done, ready for the next search...\n");
					}
					free(*(tokens + ii));
				}
				free(tokens);
