...
int main(int argc, char** argv) 
{
	...
	int amount_str=0;  //rows
	int len_str=0; //max columns for the case of not rectangle table
	//countTableElemFromFile(name, &len_str, &amount_str);
	int cols=amount_str;
	int currLine=0;
	char* outFile;
	//get file list and count clasters inside
	//int MakeListOfDir(char* DirName,char **nameList)
	...
	{	
		...
		while ((dir = readdir(d)) != NULL) 
		{
			pch=strstr(dir->d_name, input);
		  if ( !( pch || !strcmp(".",dir->d_name) || !strcmp("..",dir->d_name) ) )
		  {
			filesNum++;
			printf("Count %s,num %d\n",dir->d_name,filesNum);
		  }
		  else
			printf("do not count %s,num %d\n",dir->d_name,filesNum);
		}
		closedir(d);
	}
	char** fileList=(char**)malloc(sizeof(char*) * (filesNum));
	...
	{	iiii=0;
		while ((dir = readdir(d)) != NULL) 
		{
		  ...
		  if ( !( pch  || !strcmp(".",dir->d_name) || !strcmp("..",dir->d_name) ) )
		  { 
			fileList[iiii] = (char*) malloc(strlen(dir->d_name)+1);
			strcpy(fileList[iiii],dir->d_name);
			iiii=iiii+1;
		  }
		}
		closedir(d);
	}
	//MAY BE CHANGE fill after cluster analysys it must be summary of all present size clusters, but no more
	//ones more to copy file list
	//put results to a directory name_NameofFile, here each file for its cluster size plus statistiks file
	/*
	d = opendir(".");
	FILE* Res=fopen("ClustDistrGen.txt","w");
			int maxClust=70000;
				//print only cluster size x-axis
			for( int i = 1; i <= maxClust; i++ ) 
			{	
				if (i <= 50) 					{						fprintf(Res,"%d\t",i);}
				else if ( i>50 && i<=100)		{ if (i % 5 == 0)		fprintf(Res,"%d\t",i);}
				else if ( i>100 && i<=1000) 	{ if (i % 10 == 0) 		fprintf(Res,"%d\t",i);}
				else if ( i>1000 && i<=10000)	{ if (i % 500 == 0) 	fprintf(Res,"%d\t",i);}
				else if ( i>10000 && i<=60000) 	{ if (i % 5000 == 0) 	fprintf(Res,"%d\t",i);}
				else if ( i>60000 && i<=70000) 	{ if (i % 1000 == 0) 	fprintf(Res,"%d\t",i);}
			}
			fprintf(Res,"\n"); 
	
	*/	
	for ( int ii = 0; ii < filesNum; ++ii ) 
  ////////////////////////////
	{				
			clust=0;
			clustSingle=0;
			...
			amount_str=0;
			len_str=0;
			countTableElemFromFile(fileList[ii], &len_str, &amount_str);
			FILE* mf=fopen(fileList[ii],"r");
			DeleteExt(fileList[ii]);
			asprintf(&outFile,"%s.stat",fileList[ii]); 	
			FILE* rf=fopen(outFile,"w");
			if (mf == NULL) return 0;
//count entire elem 
			...
			{	
				//currLine=i+1;
				countTableElemFromOpendFile(mf, &len_str);
				//amount of elements
				clust=clust+(size_t)(len_str-1);
			}
			rewind(mf);
			fprintf(rf,"Elements %zu\n",clust);
			//why clust+1? clust-1 (!?)
			//what is disritb float ?
			//what is present char ?
			//what is disCount of dis
			int* distrib=(int*)calloc((clust+1),sizeof(int));
			//memset(distrib,0,(clust+1)*sizeof(int));
			//char** persent=(char**)malloc(sizeof(char*) * (clust+1));
			//memset(persent,0,(clust+1)*sizeof(char*));
			//0 - arab 1 - arab+zm 2 - zm
			//typedef int[3] dis; 
			dis* disCount=(dis*)calloc((clust+1),sizeof(dis));
			
			//here to analyse with gene Arabidobsis or Zeom
			//int IsArabOrZea(char* str) 0 - arab Zm - 1
 			
			for( int i = 0; i < amount_str; ++i ) 
			{	
				currLine=i+1;
				int res=countTableElemAndDistribArbAndZm(mf, &len_str);
				int ClusterSize=len_str-1;
				//
				if ( (ClusterSize) > 1) 
				{	
				disCount[ClusterSize][0]=disCount[ClusterSize][0]+(int)(res == 4); //arab
				disCount[ClusterSize][1]=disCount[ClusterSize][1]+(int)(res == 2); //arab+zm
				disCount[ClusterSize][2]=disCount[ClusterSize][2]+(int)(res == 1); //zm
				
				//distrib[len_str-1]=distrib[len_str-1]+(float)1;
				//number of clusters of ClusterSize
				distrib[ClusterSize]=distrib[ClusterSize]+1;
				
				//fprintf(rf,"cluster %d\t%d\t%.4f%%\n",currLine,(len_str-1),((float)(len_str-1))/((float)clust));
				}
				else if( ClusterSize == 0) fprintf(rf,"something wrong with cluster in line %d ???\n", currLine);

			}
			
			//print distrubution 
			//disCount[ClusterSize][0]=disCount[ClusterSize][0]+res & 4; //arab
			//disCount[ClusterSize][1]=disCount[ClusterSize][1]+res & 2; //arab+zm
			//disCount[ClusterSize][2]=disCount[ClusterSize][2]+res & 1; //zm
			//distrib[ClusterSize]=distrib[ClusterSize]+1; //amount of cluser of ClusterSize
			fprintf(rf,"cluster size\tnumber of clusters\tarab%%\tarab+zm%%\tzm%%\n");
			for( int i = 2; i <= clust; i++ ) 
			{	
				if (distrib[i] == 0) continue;
				//cluster size, n=number of clusters of such size, arab/n, arab+zm / n, zm /n  
				float ar = 100.0*(float)disCount[i][0] / (float)distrib[i];
				float common = 100.0*(float)disCount[i][1] / (float)distrib[i];
				float zm = 100.0*(float)disCount[i][2] / (float)distrib[i];
				fprintf(rf,"%d\t%d\t%.2f\t%.2f\t%.2f\n",i,distrib[i],ar,common,zm);
			}
			fprintf(rf,"\n");			
