...
int main(int argc, char** argv) 
{
	...
	sockfd = make_client(argv[1]);
	char *gene,*weight,*destin;
	//asprintf(&gene,"%s",argv[2]);
	asprintf(&destin,"%s",argv[3]);
	DeleteLastN(destin);
	//float thrashold=(float)atof(argv[4]);
	//asprintf(&recvBuff,"%s\t%s\t%s",argv[2],destin,argv[4]);
	...
	else
    {
        ...
        }
		{
				...
				//bzero(buffer,256);
				memset(recvBuff, '0', sizeof(recvBuff)); 
				sprintf(&recvBuff,"%s\t%s\t%s",argv[2],destin,argv[4]);
				printf("client talks:%s length=%d\n",recvBuff,strlen(recvBuff));
				n = write(sockfd,recvBuff,sizeof(recvBuff));
				if (n < 0) error("ERROR sending gene and weight to socket");
		}
    } 
	free(destin);
	//free(recvBuff);
