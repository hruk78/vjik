...
int main(int argc, char** argv) 
{
	...
	TStColAndFloat destinData;
	memset(&sourceData, 0, sizeof(sourceData));
	memset(&destinData, 0, sizeof(destinData));

	//let us read 
	ReadTwoKeyColAndFloat(source, &sourceData);
	ReadTwoKeyColAndFloat(destin, &destinData);	
	size_t Item=0;
	int present=0;
	//
		for (size_t i=0;i < sourceData.size ; i++)
		{
			...
			char* toSearchFor1 = (char*)malloc(strlen(sourceData.StrCol1[i])*sizeof(char));
			char* toSearchFor2 = (char*)malloc(strlen(sourceData.StrCol2[i])*sizeof(char));
			strcpy(toSearchFor1, sourceData.StrCol1[i]);
			strcpy(toSearchFor2, sourceData.StrCol2[i]);
			present=-1;
			if (i==0) present = FindKeyInTwoStrFloat(Item,toSearchFor1,toSearchFor2,&destinData,&Item);
			else present = FindKeyInTwoStrFloat(Item+1,toSearchFor1,toSearchFor2,&destinData,&Item);
			if (present > -1)
			{
				destinData.weight[Item]=sourceData.weight[i];
				printf("StrCol1[%zu]=%s\tStrCol2[%zu]=%s\tReplace old wght=%.2f\t with new %.2f\n",i,sourceData.StrCol1[i],i,sourceData.StrCol2[i],destinData.weight[Item],sourceData.weight[i]);

			}
			else if (present == -1)
			{
				//insert to destindata new pair of to destinData
				//Insert row to the array...do I need realloc all 
				printf("Insert StrCol1[%zu]=%s\tStrCol2[%zu]=%s\twght=%.2f\tto Dest %zu\n",i,sourceData.StrCol1[i],i,sourceData.StrCol2[i],sourceData.weight[i],Item);
				//InsertRowInTwoStringFloat(char* StrCol1, char* StrCol2, float weight,struct TStColAndFloat *src, size_t* pos)
				//destinData=*InsertRowInTwoStringFloat(toSearchFor1,toSearchFor2, sourceData.weight[i],&destinData,Item);
				//InsertRowInTwoStringFloat2 is much more optimal
				InsertRowInTwoStringFloat2(toSearchFor1,toSearchFor2, sourceData.weight[i],&destinData,Item);
			}
			free(toSearchFor1);			
			free(toSearchFor2);			
		}
		
	WriteTwoKeyColAnd2dFloatColToFile("mediana2.txt", destinData.StrCol1, destinData.StrCol2, destinData.weight, destinData.size);	
		
	FreeTStColAndFloat(&destinData);
