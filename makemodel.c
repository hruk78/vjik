...
int main(int argc, char** argv) 
{
	...
	{	
		...
		//printf("Wrong number of parametrs. Must be 19:\n");
	...
	//else
	{
		...
		asprintf(&max_depth, "%s", argv[1]);
		
		asprintf(&eta, "%s", argv[2]);
		
		asprintf(&subsample, "%s", argv[3]);
		
		asprintf(&colsample_bytree, "%s", argv[4]);
		
		asprintf(&colsample_bylevel, "%s", argv[5]);
		
		asprintf(&min_child_weight, "%s", argv[6]);
		
		asprintf(&gamma, "%s", argv[7]);
		
		asprintf(&alpha, "%s", argv[8]);
		asprintf(&lambdaParam, "%s", argv[9]);
		numIterations = atoi(argv[10]);
		asprintf(&eval_metric, "%s", argv[11]);
		asprintf(&scale_pos_weight, "%s", argv[12]);
		
		asprintf(&sNthread,"%s",argv[13]);
 
		asprintf(&modelNam, "%s", argv[14]);
		asprintf(&species1File, "%s", argv[15]);
		asprintf(&species2File, "%s", argv[16]);
		asprintf(&foldsDir, "%s", argv[17]);
		asprintf(&negFoldsDir, "%s", argv[18]);
		//char* isHeader1 = argv[18];
		//char* isHeader2 = argv[19];
		for( int i = 0; i < argc; ++i ) 
			printf( "argv[ %d ] = %s\n", i, argv[ i ] );
	}
	//fflush(NULL);
   ...
   if (getcwd(cwd, sizeof(cwd)) != NULL) {
	...
	printf("countTableElemFromFile(species1File, &len_str_max, &amount_str)\n");
	countTableElemFromFile(species1File, &len_str_max, &amount_str);
	//countTableElemFromFile("ATH_TMAP_TGR_star.txt", &len_str_max, &amount_str);
	//printf("Rows(amount_str)=%d Columns(len_str_max)=%d\n",amount_str,len_str_max);
	...
	if (sp1Data)
	{
		...
		if (sp1Data[i]) printf("Allocation for Sp1Data elements is done\n");
	}
	...
	countTableElemFromFile(species2File, &len_str_max2, &amount_str2);
	//printf("Rows(amount_str2)=%d Columns(len_str_max2)=%d\n",amount_str,len_str_max);
	...
	{
		printf("Allocation for Sp1Data is done\n");
		if (sp1Data2[i]) printf("Allocation for Sp1Data elements is done\n");
	}

	//////////////////////////////////////////////////////////////////////////
	//read file to Sp1Stolbec and sp1Data/////////////////////////////////////
	//len_str_max - real number of col, raw - amount_str
	//ReadKeyColAndIntTableFromFile("AMB_TMAP_TGR_star.txt", sp1Stopbec2, sp1Data2, &len_str_max2, &amount_str2);
	ReadKeyColAndIntTableFromFile(species2File, sp1Stopbec2, sp1Data2, &len_str_max2, &amount_str2);
	printf("Rows(amount_str)=%d Columns(len_str_max)=%d from %s\n",amount_str,len_str_max,species2File);
	...
	//read positive results .orthopairs
	//dir and files names
	char **fileCurrFold[10]; 
	char **fileCurrNegFold[10]; 
	//char HomeTaskDir[]="/mnt/lustre/hruk/TEST_PELR_SCRIPT";
	//char IterDir[]="/iter_0/results/data_for_learning/folds";
	//char NegIterDir[]="/iter_0/results/data_for_learning/negative_folds";
	// /results/data_for_learning/negative_folds
	//fileCurrFold = open(foldsDir+"/fold_"+str(curr_fold)+".orthopairs","r")
	//foldsDir  /mnt/lustre/hruk/TEST_PELR_SCRIPT
	//iter_1/results/data_for_learning/folds
	


	TStCol gNamsByFold[10];
	memset(&gNamsByFold, 0, sizeof(gNamsByFold));
	TStCol gNamsByNegFold[10];
	memset(gNamsByNegFold, 0, sizeof(gNamsByNegFold));
	int amount_str7=0;  //rows
	int len_str_max7=0; //max columns for the case of not rectangle table
	int amount_str8=0;  //rows
	int len_str_max8=0; //max columns for the case of not rectangle table
	
	for (int curr_fold=0;curr_fold<10;curr_fold++)
	{
		//asprintf(&fileCurrFold[curr_fold], "%s%s%s%d%s", HomeTaskDir,IterDir,"/fold_",curr_fold,".orthopairs"); 
		asprintf(&fileCurrFold[curr_fold], "%s%s%d%s", foldsDir,"/fold_",curr_fold,".orthopairs"); 
		//read file/////////////////////////////////////
		//file to count, a result: amount_str - rows, len_str_max - columns
		countTableElemFromFile(fileCurrFold[curr_fold], &len_str_max7, &amount_str7);
		//allocate memory for two string arrays
		//gNamsByFold[curr_fold].size=amount_str7;
		//printf ("allocated for curr_fold%d\n",curr_fold);
		gNamsByFold[curr_fold].StrCol1 = (char**)malloc(sizeof(char*) * (amount_str7));
		gNamsByFold[curr_fold].StrCol2 = (char**)malloc(sizeof(char*) * (amount_str7));
		//read from file fileCurrFold[curr_fold]
		int amount_strOld=amount_str7;
		ReadKnownTwoKeyColFromFile(fileCurrFold[curr_fold],gNamsByFold[curr_fold].StrCol1,gNamsByFold[curr_fold].StrCol2,&len_str_max7, &amount_str7,sp1Espr,sp2Espr);
		//if there is dropped pairs, reallocate memory
		if (amount_strOld != amount_str7)
		{
			gNamsByFold[curr_fold].StrCol1 = (char**)realloc(gNamsByFold[curr_fold].StrCol1,sizeof(char*) * (amount_str7));
			gNamsByFold[curr_fold].StrCol2 = (char**)realloc(gNamsByFold[curr_fold].StrCol2,sizeof(char*) * (amount_str7));
			gNamsByFold[curr_fold].size=amount_str7;
		}
		amount_strOld=0;
		...
		//printf ("read file %d\n",curr_fold);
		//printf ("delete last symbol %d\n",curr_fold);
		
		//delete last /n
		for (int a=0;a<amount_str7;a++)
		{
			DeleteLastN(gNamsByFold[curr_fold].StrCol2[a]);
			DeleteLastN(gNamsByFold[curr_fold].StrCol1[a]);
		}
				
		//negative folders
		//asprintf(&fileCurrNegFold[curr_fold], "%s%s%s%d%s", HomeTaskDir,NegIterDir,"/fold_",curr_fold,".orthopairs"); 
		asprintf(&fileCurrNegFold[curr_fold], "%s%s%d%s", negFoldsDir,"/fold_",curr_fold,".orthopairs"); 
		countTableElemFromFile(fileCurrNegFold[curr_fold], &len_str_max8, &amount_str8);
		gNamsByNegFold[curr_fold].StrCol1 = (char**)malloc(sizeof(char*) * (amount_str8));
		gNamsByNegFold[curr_fold].StrCol2 = (char**)malloc(sizeof(char*) * (amount_str8));
		//printf ("allocated for curr_fold%d\n",curr_fold);
		//read from file fileCurrFold[curr_fold]
		...
		if (amount_strOld != amount_str7)
		{
			...
			gNamsByNegFold[curr_fold].size=amount_str8;
		}
		amount_strOld=0;
		//check if there is garbage char like "\n" in the end of sp1Stopbec4
		//in future check for all possible garbage, such as spaces, \r etc
		...
		//printf ("read neg file %d\n",curr_fold);
		//printf ("delete last symbol %d\n",curr_fold);
		//delete last /n
		for (int a=0;a<amount_str8;a++)
		{
			DeleteLastN(gNamsByNegFold[curr_fold].StrCol2[a]);
			DeleteLastN(gNamsByNegFold[curr_fold].StrCol2[a]);
		}
	...
	//test purposes
	//check data by writing back to file_
	/*
	printf ("write _files from positive structure to test\n");
	for (int curr_fold=0;curr_fold<10;curr_fold++)
	{
		//asprintf(&fileCurrFold[curr_fold], "%s_", fileCurrFold[curr_fold]); 
		asprintf(&fileCurrNegFold[curr_fold], "%s_", fileCurrNegFold[curr_fold]); 
		//printf ("%s\n",fileCurrFold[curr_fold]);
		printf ("%s\n",fileCurrNegFold[curr_fold]);
		//if (!WriteTwoKeyColToFile(fileCurrFold[curr_fold],gNamsByFold[curr_fold].StrCol1,gNamsByFold[curr_fold].StrCol2,&(gNamsByFold[curr_fold].size)))
		if (!WriteTwoKeyColToFile(fileCurrNegFold[curr_fold],gNamsByNegFold[curr_fold].StrCol1,gNamsByNegFold[curr_fold].StrCol2,&(gNamsByNegFold[curr_fold].size)))			
		printf ("mistake structure to test %d\n",curr_fold);
		
	}
	*/
	//printf("TStCol posTrainSetByFold[10]\n");
	TStCol posTrainSetByFold[10];
	memset(posTrainSetByFold, 0, sizeof(posTrainSetByFold)); 
	
	//printf("TStCol negTrainSetByFold[10]\n");
	TStCol negTrainSetByFold[10];
	memset(negTrainSetByFold, 0, sizeof(negTrainSetByFold)); 
	
	//printf("TStCol posTestSetByFold[10]\n");
	TStCol posTestSetByFold[10];
	memset(posTestSetByFold, 0, sizeof(posTestSetByFold)); 

	//printf("TStCol negTestSetByFold[10]\n");
	TStCol negTestSetByFold[10];
	memset(negTestSetByFold, 0, sizeof(negTestSetByFold)); 
	
	//simple copy with known size for this two structures
	for (int foldI=0;foldI<10;foldI++)
	{
		TStColCopy(&gNamsByFold[foldI],&posTestSetByFold[foldI]);
		TStColCopy(&gNamsByNegFold[foldI],&negTestSetByFold[foldI]);
		//printf("\n gNamsByFold.size[%d] \n",foldI,gNamsByFold[foldI].size);
		//printf("\n gNamsByNegFold.size[%d] \n",foldI,gNamsByNegFold[foldI].size);
	}
	//printf("\n_gNamsByFold.size_\n");
	
	//try wihtout count structure size, but with reallocation
	for (int foldI=0;foldI<10;foldI++)
	{
		//printf("foldI=%d ",foldI);
		for (int foldJ=0;foldJ<10;foldJ++)
		{ //printf("foldJ=%d ",foldJ);
		  if (foldI != foldJ)
		  {
ApndTStColTSColl(gNamsByFold[foldJ].StrCol1,gNamsByFold[foldJ].StrCol2,gNamsByFold[foldJ].size,&posTrainSetByFold[foldI]);
//printf("positive copied ");
ApndTStColTSColl(gNamsByNegFold[foldJ].StrCol1,gNamsByNegFold[foldJ].StrCol2,gNamsByNegFold[foldJ].size,&negTrainSetByFold[foldI]);
//printf("negative copied\n");
		  }
		}
	}
	
	//PRINT FILE TO TEST ok
	//for (int foldI=0;foldI<10;foldI++)
	//{
	//	FPrintTwoStringStructure("posTrainSetByFoldRes",&posTrainSetByFold[foldI]);
	//	printf("\n\n");
	//}
	
	TStCol posAllTrainList;
	memset(&posAllTrainList, 0, sizeof(posAllTrainList)); 

	TStCol negAllTrainList;
	memset(&negAllTrainList, 0, sizeof(negAllTrainList)); 
	
	for (int foldI=0;foldI<10;foldI++)
	{
		//printf("merging 1 foldI=%d ",foldI);
		MrgeTwoTStCol(&gNamsByFold[foldI],&posAllTrainList);
		//printf("merging 2 foldI=%d ",foldI);
		MrgeTwoTStCol(&gNamsByNegFold[foldI],&negAllTrainList);
	}
	
	TStCol posTrainList;
	memset(&posTrainList, 0, sizeof(posTrainList)); 
	TStCol negTrainList;
	memset(&negTrainList, 0, sizeof(negTrainList)); 
	TStCol posTestList;
	memset(&posTestList, 0, sizeof(posTestList)); 
	TStCol negTestList;
	memset(&negTestList, 0, sizeof(negTestList)); 
	...
	char* Result=NULL;
	memset(&trainSet, 0, sizeof(trainSet));		
	ArraySet TrainSetArrPrev,TrainSetArr;
	TStCol testSet;
	ArraySet TestSetArr;
	ArraySet* pTrainSetArrPrev;
	ArraySet* pTrainSetArr;

	memset(&TrainSetArr,0,sizeof(TrainSetArr));
	memset(&TrainSetArrPrev,0,sizeof(TrainSetArrPrev));
	pTrainSetArr=&TrainSetArr;
	pTrainSetArrPrev=&TrainSetArrPrev;
	
	ArraySet* pTestSetArr;
	DMatrixHandle h_train,h_test;
	...
	for (int foldI=0;foldI<10;foldI++)
	{
		...
		memset(&negTestList, 0, sizeof(negTestList)); 
		memset(pTrainSetArr, 0, sizeof(ArraySet));
		memset(pTrainSetArrPrev, 0, sizeof(ArraySet));		
		
		MrgeTwoTStCol(&posTrainSetByFold[foldI],&posTrainList);
		MrgeTwoTStCol(&negTrainSetByFold[foldI],&negTrainList);
		MrgeTwoTStCol(&posTestSetByFold[foldI],&posTestList);
		MrgeTwoTStCol(&negTestSetByFold[foldI],&negTestList);

		//trainSet = list() 
		//labelsTest = list() First Col in test`Set
		printf(" posTrainList start loading ....\n");
		printf("now: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

		//trainSet.col2 filling with sp1 and sp2 strCol and IntMtrx 		
		//trainSet.col1 labelsTrain.append(1)
		
		memset(&trainSet, 0, sizeof(trainSet));	
		//char labelsTrain='1';

		//printf("Filed PosTrainList %d strings",FillTrainSet(&posTrainList, &trainSet, sp1Espr, sp2Espr, "1"));
		//FPrintTwoStringStructure("PosTrainSet",&posTrainList);		
		printf("pTrainSetFromIntMtrx of a = %p\n", (pTrainSetArrPrev=MkArrayTrainSetFromIntMtrx(&posTrainList, sp1Espr, sp2Espr, "1", pTrainSetArrPrev)));
		printf("negTrainList start loading ...\n");
		printf("now: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		//labelsTrain='0';
		//printf("Filed NegTrainList %d strings",FillTrainSet(&negTrainList, &trainSet, sp1Espr, sp2Espr,"0"));
		printf("pTrainSetArray with negtrain of a = %p\n", (pTrainSetArr=AppndArrayTrainSetFromIntMtrx(&negTrainList,sp1Espr,sp2Espr,"0",pTrainSetArrPrev,pTrainSetArr)));
		printf("loaded: %d-%d-%d %d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

		FPrintLblIntMtrx("pTrainSetArrPrev",pTrainSetArrPrev);			
		FPrintLblIntMtrx("pTrainSetArr",pTrainSetArr);			

		// convert to DMatrix
		
		memset(&h_train,0,sizeof(DMatrixHandle));
		memset(&h_test,0,sizeof(DMatrixHandle));
		...
		DMatrixHandle eval_dmats[1] = {h_train};
		safe_xgboost(XGBoosterCreate(eval_dmats, 1, &h_booster));
		//it is from parametrs

		safe_xgboost(XGBoosterSetParam(h_booster, "max_depth", max_depth));
		//safe_xgboost(XGBoosterSetParam(h_booster, "max_depth", "0"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "eta", eta));
		//safe_xgboost(XGBoosterSetParam(h_booster, "eta", "0.3"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "subsample", subsample));
		//safe_xgboost(XGBoosterSetParam(h_booster, "subsample", "1"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "colsample_bytree", colsample_bytree));
		//safe_xgboost(XGBoosterSetParam(h_booster, "colsample_bytree", "1"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "colsample_bylevel", colsample_bylevel));
		//safe_xgboost(XGBoosterSetParam(h_booster, "colsample_bylevel", "1"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "min_child_weight", min_child_weight));
		//safe_xgboost(XGBoosterSetParam(h_booster, "min_child_weight", "1"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "gamma", gamma));
		//safe_xgboost(XGBoosterSetParam(h_booster, "gamma", "0"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "alpha", alpha));
		//safe_xgboost(XGBoosterSetParam(h_booster, "alpha", "4"));
		//safe_xgboost(XGBoosterSetParam(h_booster, "reg_alpha", reg_alpha));

		safe_xgboost(XGBoosterSetParam(h_booster, "lambdaParam", lambdaParam));
		//safe_xgboost(XGBoosterSetParam(h_booster, "lambdaParam", "1"));
		
		safe_xgboost(XGBoosterSetParam(h_booster, "eval_metric", eval_metric));
		//safe_xgboost(XGBoosterSetParam(h_booster, "eval_metric", "auc"));			
		
		safe_xgboost(XGBoosterSetParam(h_booster, "scale_pos_weight", scale_pos_weight));
		//safe_xgboost(XGBoosterSetParam(h_booster, "scale_pos_weight", "1"));
		
		//it is not from programm parameters	
		safe_xgboost(XGBoosterSetParam(h_booster, "objective", "binary:logistic"));
		safe_xgboost(XGBoosterSetParam(h_booster, "silent", "0"));
		safe_xgboost(XGBoosterSetParam(h_booster, "nthread", sNthread));
	  ...
	  //const char* eval_result = NULL;
			printf("NumIter=%d",numIterations);
	  		//fflush(NULL);
			//getchar();
	  //for (int iii = 0; iii < n_trees; ++iii) {
		...
		//getchar();	
		char* ResultSM;
		asprintf(&ResultSM,"%s/%s/folds_%d/model/model_1_1_1_1000.test",cwd,modelNam,foldI); 
		safe_xgboost(XGBoosterSaveModel(h_booster,ResultSM));
		printf("\nsaving Save Model DMatrix %s\n",ResultSM);
		...
		//FreeTStCol(&testSet);
		FreeTStCol(&trainSet);
		//FreeArraySet(pTestSetArr);
		FreeArraySet(pTrainSetArr);
		FreeArraySet(pTrainSetArrPrev);
		FreeTStCol(&posTrainList);
		FreeTStCol(&negTrainList);
		FreeTStCol(&posTestList);
		FreeTStCol(&negTestList);
	}	
	...
	memset(&posTestList, 0, sizeof(posTestList)); 
	memset(&negTestList, 0, sizeof(negTestList)); 
	MrgeTwoTStCol(&posAllTrainList,&posTrainList);
	MrgeTwoTStCol(&negAllTrainList,&negTrainList);
	MrgeTwoTStCol(&posTestSetByFold[0],&posTestList);
	MrgeTwoTStCol(&negTestSetByFold[0],&negTestList);
	...
	//int success=XGDMatrixCreateFromMat((float *)(pTestSetArr->IntMtrx+pTestSetArr->raws*sizeof(float*)),pTrainSetArr->raws,pTrainSetArr->cols,-1,&h_train);
	safe_xgboost(XGDMatrixCreateFromMat((float *)pTrainSetArr->IntMtrx[0],pTrainSetArr->raws,pTrainSetArr->cols,0,&h_train));
	printf("\ncreating DMatrixTrain\n");
	//success=XGDMatrixCreateFromMat((float *)(pTestSetArr->IntMtrx+pTestSetArr->raws*sizeof(float*)),pTestSetArr->raws, pTestSetArr->cols, -1, &h_test);
	//safe_xgboost(XGDMatrixCreateFromMat((float *)pTestSetArr->IntMtrx[0],pTestSetArr->raws, pTestSetArr->cols, -999.9f, &h_test));
	printf("\ncreating DMatrixTest\n");

		// load the labels
	safe_xgboost(XGDMatrixSetFloatInfo(h_train, "label", (float*)pTrainSetArr->Labels, pTrainSetArr->raws));

	FPrintLblIntMtrx("pTrainSetFloat", pTrainSetArr);

 
		BoosterHandle h_booster;
		//DMatrixHandle eval_dmats[2] = {h_train, h_test};
		DMatrixHandle eval_dmats[1] = {h_train};
		//safe_xgboost(XGBoosterCreate(eval_dmats, 2, &h_booster));
		safe_xgboost(XGBoosterCreate(eval_dmats, 1, &h_booster));
		...
		safe_xgboost(XGBoosterSetParam(h_booster, "objective", "binary:logistic"));
		safe_xgboost(XGBoosterSetParam(h_booster, "silent", "0"));
		//safe_xgboost(XGBoosterSetParam(h_booster, "nthread", "2"));
	...
	char* ResultSM;
	asprintf(&ResultSM,"%s/%s/folds_all/model/model_1_1_1_1000.test",cwd,modelNam); 
	safe_xgboost(XGBoosterSaveModel(h_booster,ResultSM));
	printf("\nsaving Save Model DMatrix %s\n",ResultSM);
	...
	printf("Free TStCol posTestSetByFold...\n");
	FreeTStCol(&posAllTrainList);	
	FreeTStCol(&negAllTrainList);
		
	for (int foldI=0;foldI<10;foldI++)
		{
			FreeTStCol(&posTestSetByFold[foldI]);
			FreeTStCol(&negTestSetByFold[foldI]);
			FreeTStCol(&gNamsByFold[foldI]);
			FreeTStCol(&posTrainSetByFold[foldI]);
			FreeTStCol(&negTrainSetByFold[foldI]);
		}
	printf("TStCol posTestSetByFold is freed\n");
	printf("TStCol gNamsByFold is freed\n");
	
	for (int curr_fold=0;curr_fold<10;curr_fold++)
		{
			free(fileCurrNegFold[curr_fold]); 
		}
	
	for (int curr_fold=0;curr_fold<10;curr_fold++)
		{
			free(fileCurrFold[curr_fold]); 
		}
	...
	for (int j=0;j<amount_str;j++)
	  ...
	  free(sp1Data2);
		...
		free(eta);
		free(subsample);
		free(colsample_bytree);
		free(colsample_bylevel);
		free(min_child_weight);
		free(gamma);
		free(alpha);
		free(lambdaParam);
		free(eval_metric);
		free(scale_pos_weight);
		free(modelNam);
		free(species1File);
		free(species2File);
		free(foldsDir);
